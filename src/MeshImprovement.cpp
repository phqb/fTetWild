...
//                //pausee();
    ...
    for (int i = 0; i < tets.size(); i++) {
        ...
        if (is_inverted(mesh, i)) {
            logger().info("tet {} inverted: {} {} {} {}", i, tets[i][0], tets[i][1], tets[i][2], tets[i][3]);
            //pausee();
        }
    }
...
//                }
                ...
                for (int k = 0; k < 3; k++) {
                    if (!tet_vertices[tets[i][(j + 1 + k) % 4]].is_on_surface) {
                        ...
                        logger().info("{} {} {} {}", tets[i].is_surface_fs[0], tets[i].is_surface_fs[1], tets[i].is_surface_fs[2], tets[i].is_surface_fs[3]);
                        logger().info("{} {} {} {}", tet_vertices[tets[i][0]].is_on_surface, tet_vertices[tets[i][1]].is_on_surface, tet_vertices[tets[i][2]].is_on_surface, tet_vertices[tets[i][3]].is_on_surface);
                        //pausee();
                    }
                }
            }
            ...
            if (tets[i].is_bbox_fs[j] != NOT_BBOX) {
                for (int k = 0; k < 3; k++) {
                    if (!tet_vertices[tets[i][(j + 1 + k) % 4]].is_on_bbox) {
                        ...
                        logger().info("t {}: {} {} {} {}", i, tets[i][0], tets[i][1], tets[i][2], tets[i][3]);
                        logger().info("{} {} {} {}", (int)tets[i].is_bbox_fs[0], (int)tets[i].is_bbox_fs[1], (int)tets[i].is_bbox_fs[2], (int)tets[i].is_bbox_fs[3]);
                        logger().info("{} {} {} {}", tet_vertices[tets[i][0]].is_on_bbox, tet_vertices[tets[i][1]].is_on_bbox, tet_vertices[tets[i][2]].is_on_bbox, tet_vertices[tets[i][3]].is_on_bbox);
                        pausee();
                    }
                }
    ...
    for(int i=0;i<tet_vertices.size();i++) {
        ...
        if (tet_vertices[i].is_on_bbox) {
            ...
            if (!is_found) {
                ...
                for (int t_id:tet_vertices[i].conn_tets) {
                    ...
                    logger().info("{} {} {} {}", tets[t_id].is_bbox_fs[0], tets[t_id].is_bbox_fs[1], tets[t_id].is_bbox_fs[2], tets[t_id].is_bbox_fs[3]);
                    logger().info("{} {} {} {}", tet_vertices[tets[t_id][0]].is_on_bbox, tet_vertices[tets[t_id][1]].is_on_bbox, tet_vertices[tets[t_id][2]].is_on_bbox, tet_vertices[tets[t_id][3]].is_on_bbox);
                }
                //pausee();
            }
        }
        if (tet_vertices[i].is_on_surface) {
            ...
            if (!is_found) {
                ...
                for (int t_id:tet_vertices[i].conn_tets) {
                    logger().info("t {}: {} {} {} {}", t_id, tets[t_id][0], tets[t_id][1], tets[t_id][2], tets[t_id][3]);
                    logger().info("{} {} {} {}", tets[t_id].is_surface_fs[0], tets[t_id].is_surface_fs[1], tets[t_id].is_surface_fs[2], tets[t_id].is_surface_fs[3]);
                    logger().info("{} {} {} {}", tet_vertices[tets[t_id][0]].is_on_surface, tet_vertices[tets[t_id][1]].is_on_surface, tet_vertices[tets[t_id][2]].is_on_surface, tet_vertices[tets[t_id][3]].is_on_surface);
                }
                //pausee();
            }
        }
