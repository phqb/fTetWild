...
//        time_get_intersecting_edges_and_points2 += timer.getElapsedTime();
        if (!is_result) {
            //fortest
            logger().info("seg_plane_intersection no result!");
            logger().info("{}, {}", to_plane_dists[e[0]], to_plane_dists[e[1]]);
            logger().info("{}, {}", get_to_plane_dist(mesh.tet_vertices[v1_id].pos), get_to_plane_dist(mesh.tet_vertices[v2_id].pos));
            logger().info("e[0] = {}", e[0]);
            //fortest
            return false;
        }
...
bool floatTetWild::CutMesh::check() {
    ...
    for (auto &m:map_v_ids) {
        ...
        if (std::fabs(dist) < mesh.params.eps_coplanar && to_plane_dists[lv_id] != 0 && !is_snapped[lv_id]) {
            logger().info("wrong vertex in cut mesh");
            logger().info("{}", dist);
            logger().info("{}", to_plane_dists[lv_id]);
            logger().info("{}", static_cast<bool>(is_snapped[lv_id]));
            is_good = false;
        }
    }
    return is_good;
